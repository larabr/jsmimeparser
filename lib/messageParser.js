...
var ExtractMimeMsgEmitter = {
  ...
  endMessage() {
    ...
    const mail = {
      ...this.mimeTree,
      body: {
        html: this.mimeTree.bodyParts.html.join('<br>\n'),
        text: this.mimeTree.bodyParts.text.join('\n')
      },
    };
  ...
  endPart(partNum) {
    ...
    if (this.isAttachment(currentPart)) {
      ...
      switch(bodyType) {
        case 'text/html':
          // TODO html should be concatenated after clearing the <html> tags
