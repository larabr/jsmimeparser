...
import * as jsmime from './jsmime';
import { concatUint8Arrays, stringToTypedArray, typedArrayToString, uint8ArrayToHex } from './utils';
...
var ExtractMimeMsgEmitter = {
  ...
  endMessage() {
    ...
    if (this.options.getMimePart) {
      if (this.mimeTree.parts[0].partName == this.options.getMimePart) {
        ...
        this.mimeMsg.bodyAsTypedArray = stringToTypedArray(
          this.mimeMsg.body
        );
      }
      return;
  ...
  startPart(partNum, headerMap) {
    ...
    for (let [headerName, headerValue] of headerMap._rawHeaders) {
      ...
      headers[headerName] = valueArray.map(value => {
        ...
        return typedArrayToString(utf8ByteArray);
      });
    }
  ...
  deliverPartData(partNum, data, rawData) {
    ...
    if (typeof data === "string") {
      ...
      currentPart.body += typedArrayToString(data);
    }
...
export function parseMail(data) {
  const { headers, allAttachments, bodyParts, subject } = MimeParser.extractMimeMsg(data);
  ...
  ['subject', 'date', 'to', 'from', 'to', 'cc', 'bcc', 'message-id', 'in-reply-to', 'reply-to'].forEach(key => {
    const arrayValue = headers[key];
    if (!arrayValue) return;
    const value = singleKeys.has(key) ? arrayValue[arrayValue.length - 1] : arrayValue;
    mail[key] = value;
  })

  if (mail.date) mail.date = new Date(mail.date);

  // TODO process to/from/cc... addresses into objects? might not be needed since we re-parse the email address in pmcrypto
  return mail;
}
